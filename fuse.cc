                ...
                off_t off, struct fuse_file_info *fi)
{
    ...
    chfs_client::inum  inum = ino;
    std::string buf;
    if(chfs->read(inum, size, off, buf) != chfs_client::OK) {
        fuse_reply_err(req, ENOSYS);
    } else {
        //TODO: use c_str or data ?
                 ...
                 struct fuse_file_info *fi)
{
    ...
    int r;
    chfs_client::inum  inum = ino;
    if(chfs->write(inum, size, off, buf, size) != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
    } else {
        //TODO: use c_str or data ?
...
#if 1
    // Change the above line to "#if 1", and your code goes here
    switch (fuseserver_createhelper(parent, name, mode, &e, extent_protocol::T_DIR)) {
        case chfs_client::OK :
            fuse_reply_entry(req, &e);
            break;
